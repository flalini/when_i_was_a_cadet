...
static void	export_no_args(t_minishell *minishell)
{
	t_list	*tmp;
...
void		export_cmd3(t_minishell *minishell, t_token *args,
		char **split, int jn)
{
	int		ex;
	int		i;
...
void		export_cmd2(t_minishell *minishell, t_cmd *cmd, char **split,
					t_token *args)
{
	char	*to_free;
	int		join;
	...
	if (ft_strlen(args->word) > 0 && env_valid_character(split[0]))
		export_cmd3(minishell, args, split, join);
	else
	{
...
void		export_cmd(t_minishell *minishell, t_cmd *cmd, int forked)
{
	...
	while (args)
	{
		...
		export_cmd2(minishell, cmd, split, args);
		ft_free_split(&split);
		args = args->next;
	}
}
