...
template <typename T>
static void	print_ori_my_value(std::string const &str, T ori, T my)
{
	std::cout << GREEN << "ori " << str << " = " << ori << RESET << " ";
	std::cout << RED << " my " << str << " = " << my << RESET << std::endl;
}

static void    print_ori_my_value(std::string const &str, bool ori, int my)
{
    std::cout << GREEN << "ori " << str << " = " << ori << RESET << " ";
    std::cout << RED <<" my " << str << " = " << my << RESET << std::endl;
}
...
template <typename T>
static void    print_all_element(std::list<T> &ol, ft::List<T> &ml)
{   
    std::cout << GREEN << "[ ";
...
static bool    over_100(const int& val) { return (val >= 100); }
static bool    eqaul(const int& a, const int& b) { return (a == b); }
static bool    diff_by_1(const int& a, const int& b) { return (a - b == -1); }
...
void    list_test(void)
{
    ...
    int arr[] = {42, 18, 42, 18};
    std::list<int> range_ol;
    ft::List<int> range_ml;
	for (int i = 0; i < 4; i++) {
    	range_ol.push_back(arr[i]);
    	range_ml.push_back(arr[i]);
	}
